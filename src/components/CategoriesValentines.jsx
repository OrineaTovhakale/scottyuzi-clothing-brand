...
import { isMandelaLive } from '../utils/mandelaLive';
...
const Categories = () => {
  ...
  useEffect(() => {
    ...
    tiltEls.forEach(c => { c.addEventListener('mousemove', onMove); c.addEventListener('mouseleave', onLeave); });

    return () => {
      io.disconnect();
      window.removeEventListener('scroll', onScroll);
      window.removeEventListener('resize', onResize);
      tiltEls.forEach(c => { c.removeEventListener('mousemove', onMove); c.removeEventListener('mouseleave', onLeave); });
    };
  }, []);

  // ── CELEBRATE MANDELA DAY — full FC line-up ──────────────────────────────
  // Card price = short sleeve. Long sleeve is chosen on the PDP.
  // Every jersey — FC, Mandela Day, Bafana Bafana — R399 short / R450 long
  const FC_P = 399;
  const mandelaDrops = [
    { name: "Mandela Day FC Tee — Black", image: blackMan, hoverImage: null, price: FC_P, product: { name: "Mandela Day FC Tee — Black", image: blackMan, price: FC_P, customizable: true, isFC: true } },
    { name: "Mandela Day FC Tee — Red",   image: redMan,   hoverImage: null, price: FC_P, product: { name: "Mandela Day FC Tee — Red",   image: redMan,   price: FC_P, customizable: true, isFC: true } },
    { name: "Original Blue FC Tee",       image: fc1,      hoverImage: null, price: FC_P, product: { name: "Original Blue Scottyuzi FC tshirt", image: fc1, price: FC_P, customizable: true, isFC: true } },
    { name: "Red FC Tee",                 image: fc2,      hoverImage: null, price: FC_P, product: { name: "Red Scottyuzi FC tshirt",           image: fc2, price: FC_P, customizable: true, isFC: true } },
    { name: "Black FC Tee",               image: fc3,      hoverImage: null, price: FC_P, product: { name: "Black Scottyuzi FC tshirt",         image: fc3, price: FC_P, customizable: true, isFC: true } },
    { name: "Bonnie & Clyde FC Tee",      image: bonnie,   hoverImage: null, price: FC_P, product: { name: "ScottyUzi FC — Bonnie & Clyde Edition", image: bonnie, price: FC_P, customizable: true, isFC: true } },
    { name: "Bafana Bafana Tee — Green",  image: scXwc1,      hoverImage: null, price: FC_P, product: { name: "ScottyUzi World Cup Tee — Green", image: scXwc1,      price: FC_P, customizable: true, isWorldCup: true } },
    { name: "Bafana Bafana Tee — Black",  image: bafanaBlack, hoverImage: null, price: FC_P, product: { name: "ScottyUzi World Cup Tee — Black", image: bafanaBlack, price: FC_P, customizable: true, isWorldCup: true } },
  ];

  const preDrops = [
    { name: "I ❤️ Me Tee",            image: iLuvMe,        hoverImage: null,                price: 550, product: { name: "I ❤️ Me Tee", image: iLuvMe, price: 550 } },
    { name: "Property of SU",          image: propertySU,    hoverImage: null,                price: 500, product: { name: "Property of SU", image: propertySU, price: 500 } },
    { name: "Never 2Fly 2Pray",        image: never2Fly2Pray, hoverImage: null,               price: 550, product: { name: "Never 2Fly 2Pray", image: never2Fly2Pray, price: 550 } },
    { name: "Red Flannel Shirt",       image: redFlannel,    hoverImage: null,                price: 550, product: { name: "Red Flannel Shirt", image: redFlannel, price: 550 } },
    { name: "ScottyUzi × Blanco Tee",  image: blanco1,       hoverImage: null,                price: 450, product: { name: "ScottyUzi × Blanco Tee", image: blanco1, price: 450 } },
    { name: "Bonnie & Clyde Edition",  image: bonnie,        hoverImage: null,                price: 550, product: { name: "ScottyUzi FC — Bonnie & Clyde Edition", image: bonnie, price: 550, customizable: true } },
    { name: "Limited Edition Hoodie",  image: hoodie,        hoverImage: null,                price: 500, product: { name: "Limited Edition Hoodie", image: hoodie, price: 500 } },
    { name: "SU Wing T-Shirt",         image: suWing,        hoverImage: suWingMostLovedHover, price: 550, product: { name: "SU Wing T-Shirt", image: suWing, price: 550 } },
  ];

  const mandelaLive = isMandelaLive();
  const newDrops    = mandelaLive ? mandelaDrops : preDrops;

  const mostLovedTshirts = [
    { name: "Peel Off Tee",    image: peelOff2,    hoverImage: peelOffHover,         price: 500 },
    { name: "Essential Tee",   image: essential11, hoverImage: null,                 price: 550 },
    { name: "Est. 2020 Tee",   image: est2020,     hoverImage: est2020Hover,         price: 450 },
  ...
  const statementWords = "Redefining streetwear with bold, authentic style. Join the global movement.".split(' ');

  // collection tiles — feature / wide rhythm like the redesign
  const collectionTiles = [
    { img: originalJacketsImg,   label: "Jackets",    mod: "su-col--feat", to: '/collections?category=jackets' },
    { img: tshirtCollection,     label: "T-Shirts",   mod: "",             to: '/collections?category=t-shirts' },
    { img: hatsCollection,       label: "Hats",       mod: "",             to: '/collections?category=hats' },
    { img: tracksuitsCollection, label: "Tracksuits", mod: "su-col--wide", to: '/collections?category=tracksuits' },
  ];
    ...
    <section className="su-root" ref={sectionRef}>
      ...
      <div className="su-section su-reveal">
        <div className="su-section__header">
          <SectionLabel text={mandelaLive ? "18 July · 67 Minutes" : "Latest Arrivals"} />
          <SectionHeading>{mandelaLive ? "Celebrate Mandela Day" : "Discover What's New"}</SectionHeading>
        </div>
      ...
      {/* ── KINETIC STATEMENT (word-by-word on scroll) ──────────────── */}
      <div className="su-stmt su-reveal">
        <span className="su-label">The Ethos</span>
        <p className="su-stmt__p">
          {statementWords.map((w, i) => (<span className="su-w" key={i}><span>{w}</span></span>))}
        </p>
      </div>

      {/* ── GREEN BAFANA HERO (moved here from top of Home) ─────────── */}
      ...
      <div className="su-ticker-wrap">
        <div className="su-ticker su-ticker--dark">
          <div className="su-ticker__track su-ticker__track--fwd">
            {["FIFA WORLD CUP 2026", "SCOTTYUZI EDITION", "BLACK BAFANA BAFANA TEE", "GREEN BAFANA BAFANA TEE",
              "FIFA WORLD CUP 2026", "SCOTTYUZI EDITION", "BLACK BAFANA BAFANA TEE", "GREEN BAFANA BAFANA TEE"].map((t, i) => (
      ...
      <div className="su-section su-section--dark su-reveal">
        ...
        <div className="su-grid su-grid--2">
          <div className="su-hero-card" onClick={() => navigate('/collections?category=hats')}>
            <img src={capsMostLovedImg} alt="Hats" className="su-hero-card__img su-hero-card__img--contain" loading="lazy" />
          ...
          <div className="su-hero-card" onClick={() => navigate('/collections?category=shorts')}>
            <img src={originalShortsImg} alt="Shorts" className="su-hero-card__img" loading="lazy" />
            <div className="su-hero-card__overlay" />
      ...
      <style jsx>{`
        ...
        @media (min-width:1101px) {
          .su-grid--stagger { margin-bottom:64px; }
          .su-grid--stagger .su-card:nth-child(4n+2) { transform:translateY(56px); }
          .su-grid--stagger .su-card:nth-child(4n+3) { transform:translateY(28px); }
          .su-grid--stagger .su-card:nth-child(4n+4) { transform:translateY(64px); }
        }

        /* PRODUCT CARD */
        .su-card { cursor:pointer; background:none; border:none; overflow:visible; -webkit-tap-highlight-color:transparent; }
        ...
        .su-card__img { position:absolute; inset:0; width:100%; height:100%; object-fit:contain; padding:13%; transition:transform .7s var(--ease), opacity .4s ease; }
        ...
        .su-cols { display:grid; grid-template-columns:repeat(4,1fr); grid-auto-rows:230px; gap:16px; }
        ...
        .su-life { display:grid; grid-template-columns:repeat(12,1fr); gap:16px; align-items:start; }
